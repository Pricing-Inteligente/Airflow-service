...
TAB = "tabla_macroeconomicas"
F_PAIS = "pais"
F_VAR = "variable"
F_UNIDAD = "unidad"
F_FECHA = "fecha_scraping"
F_VALOR = "valor"
F_ANIO = None  # establecer a columna si existe, p.ej. "anio"
F_MES = None   # establecer a columna si existe, p.ej. "mes"


EMBED_MODEL = "intfloat/multilingual-e5-base"  # mismo que productos por defecto
...
def fetch_rows() -> List[Tuple]:
    base_cols = [F_PAIS, F_VAR, F_UNIDAD, F_FECHA, F_VALOR]
    opt_cols = [c for c in (F_ANIO, F_MES) if c]
    cols = ",".join(f'"{c}"' for c in base_cols + opt_cols)
...
def migrate():
    ...
    for row in rows:
        pais, var, uni, fec, val, *rest = row
        anio = rest[0] if len(rest) > 0 else None
        mes = rest[1] if len(rest) > 1 else None
